...
interface AppDataContextType {
  ...
  CeilingCalculatorOnes: ProductInfos[];
  userInfos: UserInfos;
...
export const AppDataProvider: React.FC<AppDataProviderProps> = ({ children }) => {
  ...
  const [CeilingCalculatorOnes,setCeilingCalculatorOnes] = useState<ProductInfos[]>();
  ...
  useEffect( () => {
    const fetchAppData = async () => {
      try {
        setIsLoading(true);
        const productsResponse = await axios.get(`https://${baseurl}/api/Products`);
        const products = productsResponse.data;
        setProductsInfos(products)
        setCeilingCalculatorOnes(products.filter((product: ProductInfos)=>product.product.category==="Ceiling Calculator"))
        fetchBestAnfNew(products);
        const p =products.filter((product: ProductInfos)=>product.product.category==="Ceiling Calculator")
        setData({
          products: [],
    ...
    const onCommentAdded = (payload: any) => {
      try{
        ...
        console.log("prod id : "+object.product.id)
        console.log("prod raiting : "+object.raiting)
  ...
  const onPublicNotificationReceived = (payload: any) => {
    ...
    const websocket = async () => {
          ...
          const client = new Client({
            ...
            brokerURL: `https://${baseurl}/products`,
            connectHeaders: { },
            appendMissingNULLonIncoming: true,
            webSocketFactory: () => new SockJS(`https://${baseurl}/products`), // Ensure this URL matches your server configuration
  ...
  return (
    <AppDataContext.Provider value={{ 
      data, user,token,cartElements,orders,BestProducts,ProductsInfos,NewProducts,userInfos,CeilingCalculatorOnes,fetchProductRating,loginn,fetchCart,fetchNotification,markNotificationAsRead,updateProfile,fetchOrders,deleteAccount, isLoading, error,fetchdt, logout,fetchFavorites }}> 
