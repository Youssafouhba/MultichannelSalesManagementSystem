...
const Infos = ({ fullName, email, phoneNumber, dateOfCreation }: { fullName: string, email: string, phoneNumber: string, dateOfCreation: string | Date }) => {
  ...
  const handleUpdateAccount = async () => {
    ...
    await updateProfile(userdto as UserDTO);
    setAlertVisible(true)
  };

  return (
    <ScrollView style={[styles.infos, { backgroundColor: Color.mainbackgroundcolor }]}>
      <CustomAlert visible={alertVisible} onDismiss={()=>{setAlertVisible(false),setEmailEnabled(true),setPhoneEnabled(true),setNameEnabled(true)}} duration={2000} title={"Profile Update"} message={"Congrats Your Profile Updated successfully ."} type="success"/>
